...
For optional enhancements, enqueue `assets/js/shared.js` once through an approved WordPress shared asset mechanism after the managed shell renders. If loaded after DOM ready, it initializes immediately; dynamically replaced shells can call `VelocitreeSite.init()`. The header/footer contain no inline scripts, and the menu and all FAQs still open with JavaScript disabled. No page body initializes global navigation. The custom native mobile-menu hooks intentionally avoid the legacy GitPress hamburger IDs. The only behavior the script adds to the menu is locking page scroll behind the full-screen panel; the panel itself opens, closes and fills the screen with CSS alone.

At 850px and below the header switches from `position: sticky` to `position: fixed`, with a `.vts-header-spacer` sibling holding its 74px of space in the flow. Sticky is silently cancelled by any ancestor with `overflow` or `transform` set, which mobile theme wrappers commonly apply, so fixed is the reliable choice there. The toggle is an icon-only hamburger (the word “Menu” remains as screen-reader text) at the right of the bar. While the panel is open that same `<summary>` relocates to the overlay's top-left corner and renders as a close X, so the close control is the native toggle and still works with JavaScript disabled. The panel is a full-screen overlay below the bar with centered links at `clamp(26px, 7.5vw, 34px)` and a green glow behind the “Let's connect” button. Landscape phones under 560px tall top-align the panel instead of centering it, and shift the close X up to match.

The mobile menu's type scale, tap targets and `summary` display carry `!important`, and the disclosure marker is suppressed through `list-style`, `::-webkit-details-marker` and `::marker` together. WordPress theme CSS reaches these elements through ID-scoped and `!important` rules that outrank plain class selectors; without this the live menu rendered at the theme's body font size and showed a native triangle beside the hamburger. `font-size` and the panel height also carry plain `px`/`vh` declarations ahead of their `clamp()` and `dvh` versions, so a sanitizer dropping the modern value falls back instead of inheriting. `qa_browser.py` replays the menu with representative theme CSS injected to keep this from regressing.
...
Local validation covers 7 HTML files for GitPress boundaries, IDs, asset references and navigation; 37 Chrome browser checks cover both pages at 320, 390, 768, 1024 and 1440px, with and without JavaScript, plus native menus/disclosures, Escape/outside-click closing, the pinned mobile bar and full-screen centered menu panel, menu resilience against theme CSS overrides, contact navigation, keyboard skip links, reduced-motion rendering, service deep links and the form preview boundary. Full screenshots and `qa-report.json` are in `preview/`. Browser verification can be rerun with `python velocitree/tools/qa_browser.py` after installing the optional `playwright` Python package; it uses installed Google Chrome.
