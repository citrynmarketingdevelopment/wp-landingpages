...
Displayed images ship as WebP derivatives generated from the supplied originals, which stay in the repository. The header and footer emblem comes from `vts logo only 9.11.webp`: the pure-black field is flood-filled from the edges to transparency (so dark pixels inside the artwork survive), trimmed to the content box, and written at 52x42, 104x84 and 156x126 for 1x/2x/3x `srcset`. Because the field is transparent and pre-trimmed, the emblem no longer needs the radial mask and negative-offset crop the previous square logo required. The hero comes from `VTS climber with logo in sun.webp` at 640, 960, 1280 and 1672 wide (q86 below 1280, q82 at and above). Phones get art direction instead: at 480px and under, a `<picture>` source serves the purpose-shot portrait frame `VTS climber with logo in sun-mobile.webp` (6000x10500, 4:7) as `vts-hero-mobile-720.webp` (720x1260, 70KB) and `vts-hero-mobile.webp` (1440x2520, 138KB), chosen by `sizes="100vw"` so low pixel-ratio phones take the smaller file; 1440 wide is a true 3x at the 480px top of that bracket. The 4:7 ratio matches the widest mobile hero box (480 by 840), so the full height always shows and only the sides crop -- the visible width runs from 100% at 480px down to 67% at 320px, which is the horizontal safe zone the art is composed within. The frame keeps its sky clear to 45% and places the helmet at 57%, torso to 80% and boots at 93%, so the copy and CTAs sit at the top of an 840px hero and finish by 38%, and the gradient runs dark to 38%, releases by 50% and is clear over the climber from 60% -- measured at 15:1 or better against white text across the copy band. Above 480px the width ladder applies with `sizes="(max-width: 767px) 1226px, 100vw"`, the 1226px accounting for the landscape frame's cover-crop rather than the element width. A local Lighthouse mobile audit of the earlier build scored 68 performance, 100 accessibility and 96 best practices, with 0 ms total blocking time and 0.001 layout shift; simulated LCP was 6.6 seconds, so production hosting, caching and further font optimization still need separate verification. The standalone previews intentionally remain noindex.
